...
import {DeriveTransactionResponse, fetchUnsignedTransaction, prepareTransaction, StakeIntentResponse} from "./api";
...
function getConfigForNetwork() {
    ...
    return {
        ...
        stakeApiUrl: isTestnet
            ? 'https://svc.blockdaemon.com/boss/v1/polkadot/westend/stake-intents'
...
async function main() {
    ...
    const submit: CompileAndSendResponse = await compileAndSend(deriveTransactionResponse.unsigned_tx, signature, pubKey, config.compileAndSendUrl)
